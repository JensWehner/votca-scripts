...
system("cat zotero.bib | grep -v -e url -e doi -e issn -e month >> literature.bib");

# Get the list of journal names in a variable
open(JOURNALS,  "./scripts/abbreviations.txt") or die "Can't open journalnames.txt: $!";
...
open (OUTFILE, '> literature_short.bib') or die "Can't open literature_short.bib: $!";
print OUTFILE @file;
close (OUTFILE);
