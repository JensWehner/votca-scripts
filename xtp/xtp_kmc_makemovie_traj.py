...
class carriercollection(object):
    ...
    def addtimestep(self,row):
        ...
        for carrier in self.listofcarriers:
            ...
            if np.absolute(float(row[1]))>0.001:
                ...
                carrier.append(1E9*float(row[3*i+1]),1E9*float(row[3*i+2]),1E9*float(row[3*i+3]),float(row[0]))
            i+=1
    ...
    def plottimestep(self,time,step):
        ...
        for carrier in self.listofcarriers:
            ...
            #ax.scatter(posarray[-1,1], posarray[-1,2], posarray[-1,3],s=200,marker="o",c=carrier.color)  
    ...
    def makemovie(self,length=10,framerate=25):
        self.assigncolors()
        self.boxdimension()
        ...
        os.system('ffmpeg -i frame_%04d.png video.ogv')
        
if len(sys.argv)==1:
    print "Enter just all .csv files you want to read in."
    sys.exit()
